    ...
    <div class='bg-white p-20'>
        <div class='row'>
            ...
            <div class='col-md-6 '>
                <div class="panel panel-info">
                    <div class="panel-heading">
                        <h3 class="panel-title">未分配权限</h3>
                    </div>
                    <div class="panel-body" id='<?php echo $roleId;?>-no-permission'>
                        <?if(!empty($no_lists)):?>
                        <?foreach ($no_lists as $key => $items) :?>
                            <div class='margin-b-10'><b><?php echo '用户、课程、管理';//Lang::get('text.role-title-'.$key);?> : </b></div>
                            <div id='no-permission-<?php echo $key;?>'>
                                <?if(!empty($items)):?>
                                    <?foreach ($items as $k => $item):?>
                                    <div class='role-h-item' id='<?php echo $roleId?>-<?php echo $item->id?>'>
                                        <span id='<?php echo $roleId?>-<?php echo $item->id?>-name' class='float-left'>code2</span> 
                                        <?//if(User::hasPermission('Admin','RoleController','postSetPermission')):?>
